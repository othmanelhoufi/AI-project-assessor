export class ResultAIAssistant {
  constructor() {
    this.slides = [];
    this.currentSlide = 0;
    this.idPrefix = 'ai-plan-' + Math.random().toString(36).substring(2, 9);
  }
  ...
  _renderStrategicPlan(content) {
    ...
    return `
      <div class="bg-white shadow-xl rounded-lg p-6 border border-gray-200">
        ...
        <div id="${this.idPrefix}-slideshow" class="relative">
          <div id="${this.idPrefix}-slides-container" class="bg-gray-50 p-6 rounded-md border prose prose-blue max-w-none h-[36rem] overflow-y-auto transition-opacity duration-300 ease-in-out">
            ${marked.parse(this.slides[0] || '')}
          </div>
          <div class="flex justify-between items-center mt-4">
              <div class="w-1/3">
                <button id="${this.idPrefix}-prev-btn" class="px-3 md:px-4 py-2 text-sm font-medium text-gray-600 hover:text-gray-800 disabled:text-gray-400 disabled:cursor-not-allowed transition-colors rounded-lg flex items-center">
              ...
              <div class="w-1/3 text-center">
                <div id="${this.idPrefix}-counter" class="text-sm font-medium text-gray-600">
                    Section 1 of ${this.slides.length}
                </div>
              </div>
              <div class="w-1/3 flex justify-end">
                <button id="${this.idPrefix}-next-btn" class="px-3 md:px-4 py-2 text-sm font-medium bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:bg-gray-400 disabled:cursor-not-allowed transition-colors flex items-center">
                    Next
  ...
  attachSlideshowEvents() {
    const slideshow = document.getElementById(`${this.idPrefix}-slideshow`);
    if (!slideshow) return;
    const prevBtn = document.getElementById(`${this.idPrefix}-prev-btn`);
    const nextBtn = document.getElementById(`${this.idPrefix}-next-btn`);
  ...
  _renderCurrentSlide() {
    const container = document.getElementById(`${this.idPrefix}-slides-container`);
    if (!container) return;
  ...
  _updateSlideshowControls() {
    const prevBtn = document.getElementById(`${this.idPrefix}-prev-btn`);
    const nextBtn = document.getElementById(`${this.idPrefix}-next-btn`);
    const counter = document.getElementById(`${this.idPrefix}-counter`);
